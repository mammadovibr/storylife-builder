...
- Image Studio image-variant cards have a dedicated delete control. Removal asks for
  confirmation and removes the variant from the current project without immediately
  deleting the shared source file from disk, because a legacy JSON project may still
  reference it.
...
- Desktop Image Studio shows generated-image storage statistics and its exact folder.
  ...
  project, its autosave, ten local backups, or AI project drafts. The UI warns that
  external legacy JSON files cannot be discovered automatically. Portable `.storylife`
  archives keep their own embedded copies and do not depend on the shared folder.
